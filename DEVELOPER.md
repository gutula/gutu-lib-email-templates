...
- You need **email templates**, **rendering helpers**, **transactional messaging** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/email-templates` instead of reaching into app-specific internals.
...
| `framework/libraries/email-templates` | Nested publishable library package. |
| `framework/libraries/email-templates/src` | Public runtime source and exported modules. |
| `framework/libraries/email-templates/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineEmailTemplate`, `createEmailTemplateRegistry`, `renderEmailTemplate`, `renderEmailPreview`, `RenderedEmail` |
...
- Hosts should import from `@platform/email-templates`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/email-templates"]
...
console.log(typeof defineEmailTemplate);
```
